...
def compose_final_image(base_image, title, subtitle, date_str, salmo_title, refrao, body_paragraphs):
    ...
    # --- 1. Desenha Cabeçalho (Título, Subtítulo, Data) ---
    y_cursor = 60
    # Título (com sombra)
    ...
    margin_right = 40
    draw.text((background.width - margin_right, 60), date_str, font=font_date, fill=TEXT_COLOR_HEADER, anchor="ra") # ra = right anchor

    # --- 2. Desenha a Caixa Translúcida e o Conteúdo do Salmo ---
    card_height = int(background.height * 0.6) # Caixa ocupa 60% da parte inferior
    card_margin = 40
    card_area = [(card_margin, background.height - card_height), (background.width - card_margin, background.height - card_margin)]
    
    # Cria uma camada separada para o cartão para aplicar a transparência
    overlay = Image.new('RGBA', background.size, (0,0,0,0))
    draw_overlay = ImageDraw.Draw(overlay)
    draw_overlay.rounded_rectangle(card_area, fill=(0, 0, 0, CARD_OPACITY), radius=20)
    
    # Combina a camada do cartão com o fundo
    background = Image.alpha_composite(background.convert('RGBA'), overlay).convert('RGB')
    draw = ImageDraw.Draw(background)

    # --- Desenha o texto DENTRO da caixa ---
    y_cursor = background.height - card_height + 40 # Posição inicial dentro da caixa
    x_pos = card_margin + 40
    text_area_width_pixels = background.width - (2 * (card_margin + 40))
